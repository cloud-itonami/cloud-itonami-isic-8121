(ns buildingcleaningops.governor-contract-test
  ...
  (testing "clean service-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-service-record :site-id "site-1" :patch {:areas-cleaned ["lobby"]}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-safety-concern :site-id "site-1"
                                :patch {:concern "chemical incompatibility" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :site-id "site-1"
                                :patch {:item "replacement scrubber unit" :quantity 1 :estimated-cost 3200.0}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into chemical-safety-clearance/incompatibility-override scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8b"
                     {:op :log-service-record :site-id "unknown" :patch {:areas-cleaned ["lobby"]}}
                     ctx)
