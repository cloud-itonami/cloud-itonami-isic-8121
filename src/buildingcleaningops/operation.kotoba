(ns buildingcleaningops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (BuildingCleaningAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the BuildingCleaningGovernor
  ...
  human operator/cleaning-operations coordinator. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [buildingcleaningops.advisor :as advisor]
            [buildingcleaningops.governor :as governor]
...
(defn build
  ...
  `buildingcleaningops.store/Store`).
  opts:
    :advisor      -- a `buildingcleaningops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BuildingCleaningAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; BuildingCleaningGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
